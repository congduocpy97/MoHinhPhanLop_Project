...
export class TourDuLich_Select extends PureComponent {
    constructor(props){
        super(props)
        this.state = {
            list: []
    ...
    componentDidMount(){
        Promise.all([getAll(Model_TourDuLich.table_database)]).then(result => {
            this.setState({
                list: result[0]
            })
        })
        ...
        return (
            ...
            >
                ...
                {this.state.list.map(m =>{
                    return (
                        <Option key={m.maTour} value={m.maTour} >{m.tenGoi}</Option>
                    )
                })}
            </Select>
        )
...
export default TourDuLich_Select
