...
export class DoanDuLich_Select extends PureComponent {
    constructor(props){
        super(props)
        this.state = {
            list: []
    ...
    componentDidMount(){
        Promise.all([getAll(Model_DoanDuLich.table_database)]).then(result => {
            this.setState({
                list: result[0]
            })
        })
        ...
        return (
            ...
            >
                ...
                {this.state.list.map(m =>{
                    return (
                        <Option key={m.maDoan} value={m.maDoan} >{m.maDoan}</Option>
                    )
                })}
            </Select>
        )
...
export default DoanDuLich_Select
