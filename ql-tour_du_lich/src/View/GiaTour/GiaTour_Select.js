...
export class GiaTour_Select extends PureComponent {
    constructor(props){
        super(props)
        this.state = {
            list: []
    ...
    componentDidMount(){
        Promise.all([getAll(Model_GiaTour.table_database)]).then(result => {
            this.setState({
                list: result[0]
            })
        })
        ...
        return (
            ...
            >
                ...
                {this.state.list.map(m =>{
                    return (
                        <Option key={m.maGia} value={m.maGia} >{m.maGia}</Option>
                    )
                })}
            </Select>
        )
