...
export class NhanVien_Edit extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selected: [],
            preModel: JSON.parse(JSON.stringify(Model_NhanVien.NhanVien)),
    ...
    handleOk = () => {
        if(JSON.stringify(this.state.preModel) !== JSON.stringify(this.state.searchModel)){
            Promise.all([update_model([this.state.searchModel, this.state.preModel], Model_NhanVien.table_database)]).then(result => {
    ...
    render() {
        ...
        return (
            <span>
                ...
                <Modal title="Chỉnh sửa nhân viên" visible={this.state.isModalVisible} width={800} onCancel={this.handleCancel} onOk={this.handleOk}>
                    <Card>
