...
export class LoaiHinhDuLich_Edit extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selected: [],
            preModel: JSON.parse(JSON.stringify(Model_LoaiHinhDuLich.LoaiHinhDuLich)),
    ...
    showModal = () => {
        if(this.state.selected.length !== 1){
            ...
            Promise.all([find_model(this.state.selected[0], Model_LoaiHinhDuLich.table_database)]).then(result => {
                this.setState({
    ...
    handleOk = () => {
        if(JSON.stringify(this.state.preModel) !== JSON.stringify(this.state.searchModel)){
            Promise.all([update_model([this.state.searchModel, this.state.preModel], Model_LoaiHinhDuLich.table_database)]).then(result => {
    ...
    render() {
        ...
        return (
            <span>
                ...
                <Modal title="Chỉnh sửa loại hình du lịch" visible={this.state.isModalVisible} width={800} onCancel={this.handleCancel} onOk={this.handleOk}>
                    <Card>
                        <Card.Grid hoverable={false} style={GridStyle}>
                            <Row>
                                ...
                                <Col span={LayoutFormat.ColInput}>
                                    <FormItem>
                                        <My_Input_Number disabled={true} style={InputStyle} value={searchModel.maLoaiHinh} />
                                    </FormItem>
                                </Col>
...
export default LoaiHinhDuLich_Edit