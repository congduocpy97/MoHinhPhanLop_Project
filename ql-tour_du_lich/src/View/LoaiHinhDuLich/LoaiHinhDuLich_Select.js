...
export class LoaiHinhDuLich_Select extends PureComponent {
    constructor(props){
        super(props)
        this.state = {
            list: []
    ...
    componentDidMount(){
        Promise.all([getAll(Model_LoaiHinhDuLich.table_database)]).then(result => {
            this.setState({
                list: result[0]
            })
        })
        ...
        return (
            ...
            >
                ...
                {this.state.list.map(m =>{
                    return (
                        <Option key={m.maLoaiHinh} value={m.maLoaiHinh} >{m.tenLoaiHinh}</Option>
                    )
                })}
            </Select>
        )
...
export default LoaiHinhDuLich_Select
