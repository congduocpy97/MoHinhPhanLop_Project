...
export class DiaDiem_Select extends PureComponent {
    constructor(props){
        super(props)
        this.state = {
            list: []
    ...
    componentDidMount(){
        Promise.all([getAll(Model_DiaDiem.table_database)]).then(result => {
            this.setState({
                list: result[0]
            })
        })
        ...
        return (
            ...
            >
                ...
                {this.state.list.map(m =>{
                    return (
                        <Option key={m.maDiaDiem} value={m.maDiaDiem} >{m.ten}</Option>
                    )
                })}
            </Select>
        )
