...
export class LoaiChiPhi_Edit extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selected: [],
            preModel: JSON.parse(JSON.stringify(Model_LoaiChiPhi.LoaiChiPhi)),
    ...
    handleOk = () => {
        if(JSON.stringify(this.state.preModel) !== JSON.stringify(this.state.searchModel)){
            Promise.all([update_model([this.state.searchModel, this.state.preModel], Model_LoaiChiPhi.table_database)]).then(result => {
    ...
    render() {
        ...
        return (
            <span>
                ...
                <Modal title="Chỉnh sửa loại chi phí" visible={this.state.isModalVisible} width={800} onCancel={this.handleCancel} onOk={this.handleOk}>
                    <Card>
...
export default LoaiChiPhi_Edit